...
{
public:
    static void arrange(ArrangePolygons& items, const ArrangePolygons& excludes, const BoundingBox& bed, const ArrangeParams& params)
    {
        if (items.empty())
            return;
        ...
        for (auto& ex : excludes) {
            ...
            if (ex.rotation != 0.0)
                epoly.rotate(ex.rotation);
            epoly.translate(ex.translation.x() - ebed.min.x(), ex.translation.y() - ebed.min.y());
            if (ex.inflation > 0) {
        ...
        for (size_t oi = 0; oi < order.size(); ++oi) {
            ...
            const auto& rots = !item.allowed_rotations.empty() ? item.allowed_rotations : default_rotations;
            ...
            for (double rot : rots) {
                ...
                if (pad_px > 0) {
                    ...
                    if (iw > bw || ih > bh)
                        continue;
                    iwpr_item = (iw + 63) / 64;
                    ibm       = dilate_bitmap(raw_bm, raw_iwpr, raw_iw, raw_ih, pad_px, iwpr_item, iw, ih);
                ...
                rot_cache.push_back({rot, std::move(ibm), inflated_bb, iw, ih, iwpr_item, coarse});
            }
                        ...
                        for (int py = 0; py <= max_py; py += rc.coarse) {
                            ...
                            for (int pass_y = 0; pass_y < 2; ++pass_y) {
                                ...
                                for (int px = 0; px <= max_px; px += rc.coarse) {
                                    if (!collides(plates[plate_idx], wpr, bw, bh, rc.bm, rc.iwpr, rc.iw, rc.ih, px, test_py)) {
                                        found_px = px;
                                        found_py = test_py;
                                        goto coarse_hit;
                                    }
                                    ...
                                    if (px + rc.coarse > max_px && px < max_px) {
                                        if (!collides(plates[plate_idx], wpr, bw, bh, rc.bm, rc.iwpr, rc.iw, rc.ih, max_px, test_py)) {
                                            found_px = max_px;
                                            found_py = test_py;
                                            goto coarse_hit;
                                        }
                    ...
                    // Phase 2: refine within ±coarse of the coarse hit
                    {
                        ...
                        for (int py = ry0; py <= ry1; ++py) {
                            for (int px = rx0; px <= rx1; ++px) {
                                if (!collides(plates[plate_idx], wpr, bw, bh, rc.bm, rc.iwpr, rc.iw, rc.ih, px, py)) {
                                    best_px          = px;
                                    best_py          = py;
                                    best_rot         = rc.rot;
        ...
        done_searching:
            ...
            if (placed) {
                ...
                stamp(plates[best_plate], wpr, bw, bh, best_bm, best_iwpr, best_iw, best_ih, best_px, best_py);
                ...
                double origin_x = best_px * res - unscaled<double>(best_inflated_bb.min.x()) + unscaled<double>(ebed.min.x());
                double origin_y = best_py * res - unscaled<double>(best_inflated_bb.min.y()) + unscaled<double>(ebed.min.y());
        ...
        if (params.do_final_align) {
            ...
            for (int plate = 0; plate <= current_plate; ++plate) {
                if (plate_has_exclude[plate])
                    continue;

                BoundingBox cluster_bb;
                bool        has_item = false;
                for (auto& item : items) {
                    if (item.bed_idx != plate)
                        continue;
                    ExPolygon placed = item.poly;
                    if (item.rotation != 0.0)
                        placed.rotate(item.rotation);
...
private:
    ...
    static std::vector<uint64_t> dilate_bitmap(const std::vector<uint64_t>& src, int swpr, int sw, int sh, int pad, int dwpr, int dw, int dh)
    {
        ...
        for (int y = 0; y < dh; ++y) {
            ...
            for (int step = 1; remaining > 0; step *= 2) {
                ...
                for (int w = dwpr - 1; w >= 0; --w) {
                    ...
                    uint64_t shifted = (bit_shift == 0) ? lo : (lo << bit_shift) | (hi >> (64 - bit_shift));
    ...
    static void stamp(
        std::vector<uint64_t>& plate, int wpr, int bw, int bh, const std::vector<uint64_t>& item_bm, int iwpr, int iw, int ih, int px, int py)
    {
        assert(px >= 0 && py >= 0);
        for (int y = 0; y < ih; ++y) {
            int by = py + y;
    ...
    static std::vector<uint64_t> rasterize(const ExPolygon& poly, double res_mm, int max_w, int max_h, int& iw, int& ih, int& iwpr)
    {
    ...
    static void scanline_fill(
        const Polygon& poly, const BoundingBox& bb, double res_mm, int iw, int ih, int iwpr, std::vector<uint64_t>& bm, bool set)
    {
        ...
        for (int y = 0; y < ih; ++y) {
            ...
            for (size_t k = 0; k + 1 < xs.size(); k += 2) {
                ...
                if (w0 == w1) {
                    ...
                    uint64_t mask = ((b1 - b0 + 1) == 64) ? ~uint64_t(0) : ((uint64_t(1) << (b1 - b0 + 1)) - 1) << b0;
                    ...
                    // Last partial word
                    uint64_t tail = (b1 == 63) ? ~uint64_t(0) : (uint64_t(1) << (b1 + 1)) - 1;
